...
function dropEverything(pool: PoolConnection, logs?: boolean): void {
    const query = fs.readFileSync(pathToStarterQueries('drop_everything.sql'), 'utf8');
    ...
    pool.beginTransaction((transaction_error) => {
        ...
        pool.query(query, (query_error, results) => {
            ...
            pool.commit((commit_error) => {
                ...
                console.log('Drop everything succesfully!');
            });
        })
    });
};
